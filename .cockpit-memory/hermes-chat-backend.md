...
- **Chat model is explicit:** `HERMES_MAIN_MODEL = deepseek/deepseek-v4-pro` from `shared/hermes-model-policy.ts`. Mechanical background services use the separate Flash constant; changing Hermes's config default cannot silently swap these roles.
