...
describe('ChatService memory foundation', () => {
  it('injects task-relevant project memory into every Claude chat request', async () => {
    const projects = {
      get: vi.fn(() => ({ id: 'prj_1', name: 'cockpiT', path: '/tmp/prj' })),
    } as unknown as ProjectService
    const memory = {
      forTask: vi.fn(() => ({
        block: 'COCKPIT PROJECT MEMORY\nstatus: ready\nUse molten obsidian surfaces.',
        receipt: {
    ...
    expect(prompt).toContain('Use molten obsidian surfaces.')
