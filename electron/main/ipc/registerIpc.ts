...
export function registerIpc(services: Services): void {
  ...
  handle('councilRun', (p) => {
    const { projectId, model, mode, dir, question, spec, cardId, responseLanguage, analysisEgress, analysisConsent, clientRunId } = councilRunSchema.parse(p)
    return services.council.run(projectId, { model, mode, dir, question, specText: spec, cardId, responseLanguage, analysisEgress, analysisConsent, clientRunId })
  })
